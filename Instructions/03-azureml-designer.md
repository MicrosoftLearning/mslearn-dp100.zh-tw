...
ms.locfileid: "146733073"
---
# <a name="use-azure-machine-learning-designer"></a>使用 Azure Machine Learning 設計工具

Azure Machine Learning *設計工具* 提供了一個能夠拖放檔案的環境，讓您可以在其中定義工作流程，或是資料擷取、轉換和模型定型元件的 *管線*，以建立機器學習模型。 您可以接著將此管線發佈為 Web 服務，讓用戶端應用程式可用來進行 *推斷* (從新資料產生預測)。
...
1. 在 Azure Machine Learning 工作室中，檢視 [資料] 頁面。 資料集代表您計劃在 Azure ML 中使用的特定資料檔案或資料表。
...
2. 按一下右側的 **&#9881;** 圖示以開啟 [設定] 窗格，將預設管道名稱 (**Pipeline-Created-on-* date***) 變更為 **Visual Diabetes Training**。
3. 請注意，您必須指定要對其執行管線的計算目標。 在 [設定] 窗格中，按一下 [選取計算類型]，然後選取 [計算叢集]，按一下 [選取 Azure ML 計算叢集]，然後選取您的電腦叢集並關閉 [設定]。
4. 在設計工具的左側，選取 [資料] 分頁，然後將 [糖尿病資料集] 資料集拖曳至畫布。
5. 選取畫布上的 [糖尿病資料集] 元件。 然後以滑鼠右鍵按一下，再選取 [預覽資料]。
6. 在 [DataOutput] 窗格中，選取 [設定檔] 索引標籤。
7. 檢閱資料的結構描述，請注意，各種資料行的分佈會以長條圖表示。 關閉視覺效果。

## <a name="add-transformations"></a>新增轉換

一般而言，您必須先對資料套用一些前置處理轉換，才可為模型定型。

1. 在左側窗格中，選取 [元件] 分頁，其中包含在模型定型前可用來轉換資料的各種元件。 您可以在窗格頂端搜尋元件。
2. 搜尋 [正規化資料] 元件並拖曳至畫布，放在 [糖尿病資料集] 元件下方。 然後將 [糖尿病資料集] 元件的輸出連線至 [正規化資料] 元件的輸入。
3. 選取 [正規化資料] 元件並檢視其設定，請注意，您必須指定轉換方法和要轉換的資料行。 然後，將轉換保持在 **ZScore** 不變，並編輯資料行以包含下列資料行名稱：
    * PlasmaGlucose
...
4. 現在，我們已準備好將資料分割成不同的資料集，以進行定型和驗證。 在左側窗格中，搜尋 [分割資料] 元件並拖曳至畫布上 [正規化資料] 元件底下。 然後，將 [正規化資料] 元件的 [已轉換的資料集] 輸出 (左側) 連線至 [分割資料] 元件的輸入。
5. 選取 [分割資料] 元件，然後設定其設定，如下所示：
    * **分割模式**：分割資料列
    * **第一個輸出資料集中的資料列比例**：0.7
    * **隨機種子**：123
    * **分層的分割**：否

## <a name="add-model-training-components"></a>新增模型定型元件

備妥資料並分割成定型和驗證資料集之後，您就可以設定管線來定型和評估模型。

1. 搜尋 [定型模型] 元件並拖曳至畫布上 [分割資料] 元件底下。 然後，將 [分割資料] 元件的 *Result dataset1* (左側) 輸出連線至 [定型模型] 元件的 [資料集] (右側) 輸入。
2. 我們要定型的模型會預測 [糖尿病] 值，因此請選取 [定型模型] 元件並修改其設定，將 [標籤資料行] 設定為 [糖尿病] (大小寫和拼字都要完全符合！)
3. 模型將預測的 **糖尿病** 標籤是一個二進位資料行 (1 表示罹患糖尿病的病患，0 表示沒有患病的病患)，因此我們需要使用 *分類* 演算法來定型模型。 搜尋 [二級羅吉斯迴歸] 元件並拖放至畫布上 [分割資料] 元件左側，且放在 [定型模型] 元件上方。 然後，將其輸出連線至 [定型模型] 元件的 [未定型模型] (左側) 輸入。
4. 若要測試定型的模型，必須用此模型來評分分割原始資料時所保留的驗證資料集。 搜尋 [評分模型] 元件並拖放至畫布上 [定型模型] 元件下方。 然後將 [定型模型] 元件的輸出連線至 [評分模型] 元件的 [定型模型] (左側) 輸入，將 [分割資料] 元件的 [結果資料集2] (右側) 輸出拖曳到 [評分模型] 元件的 [資料集] (右側) 輸入。
5. 若要評估模型的執行狀況，我們需要查看一些透過評分驗證資料集所產生的計量。 搜尋 [評估模型] 元件並拖曳至畫布上 [評分模型] 元件底下，並將 [評分模型] 元件的輸出連線至 [評估模型] 元件的 [評分資料集] (左側) 輸入。
...
3. 在 [正規化資料] 元件完成後請選取該項，然後在 [設定] 窗格中，[輸出 + 記錄] 索引標籤 [轉換資料集] 區段內的 [資料輸出] 底下，按一下 [預覽資料] 圖示，這時您可以檢視已轉換資料行的統計資料和分佈視覺效果。
4. 關閉 [正規化資料] 視覺效果，並等候其餘元件完成。 接著將 [評估模型] 元件的輸出視覺化，以查看模型的計量。

    **注意**：此模型的效能不盡理想，部分原因是我們僅執行了最基本的特徵工程和前置處理。 您可以試著使用不同的分類演算法，並比較結果 (您可以將 [分割資料] 元件的輸出連線至多個 [定型模型] 和 [評分模型] 元件，也可以再將另一個已評分的模型連線至 [評估模型] 元件，以兩相比較)。 本練習的重點是介紹設計工具介面，而不是定型出完美的模型！

## <a name="create-an-inference-pipeline"></a>建立推斷管線

既然您已使用過 *定型管線* 來定型模型，您可以建立 *推斷管線*，以使用定型的模型來預測新資料的標籤。

1. 在 [作業] 索引標籤中，瀏覽至已完成的管線。 
2. 選取 [建立推斷管線]，然後按一下 [即時推斷管線]。 幾秒鐘後，就會開啟名為 **視覺化糖尿病定型 - 即時推斷** 的新版管線。
3. 將新管線重新命名為 **Predict Diabetes**，然後檢閱新的管線。 請注意，此管線封裝了正規化轉換和已定型模型，因此訓練資料中的統計資料會用來正規化所有新資料值，而定型的模型則會用來對新資料評分。
4. 請注意推斷管線假設新資料會符合原始訓練資料的結構描述，因此會包含來自訓練管線的 **糖尿病資料集** 資料集。 不過，此輸入資料包含模型所預測的 [糖尿病] 標籤，而難以直接納入尚未進行糖尿病預測的新患者資料中。
5. 從推斷管線中刪除 [糖尿病資料集] 資料集，並將其取代為 [手動輸入資料] 元件；將其連接至 [套用轉換] 元件中，與 [Web 服務輸入] 相同的 [資料集] 輸入。 然後修改 [手動輸入資料] 元件的設定，以使用下列 CSV 輸入，其中包含了不帶標籤的三個新病患觀察的特徵值：
...
5. 推斷管線包含 [評估模型] 元件，這在使用新資料預測時沒什麼用處，因此請刪除此元件。
6. [評分模型] 元件的輸出包含所有輸入特徵，以及預測的標籤和機率分數。 若要將輸出限制為只有預測和機率，請刪除 [評分模型] 元件與 [Web 服務輸出] 之間的連線，並新增 [執行 Python 指令碼] 元件。 將 [評分模型] 元件的輸出連線至 [執行 Python 指令碼] 的 **Dataset1** (最左邊) 輸入，並將 [執行 Python 指令碼] 元件的輸出連線至 [Web 服務輸出]。 然後修改 [執行 Python 指令碼] 元件的設定，以使用下列程式碼 (取代所有現有的程式碼)：
...
```
> **注意**：在 [執行 Python 指令碼] 元件中貼上程式碼之後，請確認程式碼看起來與上述程式碼類似。 縮排在 Python 中相當重要，如果縮排未正確複製，元件將會失敗。 

7. 確認您的管線顯示類似如下：

    ![視覺化推斷管線](images/visual-inference.jpg)

9. 在用於定型的計算叢集上，將管線提交為名為 **mslearn-designer-predict-diabetes** 的新實驗。 這可能需要一些時間！
...
1. 如果 **預測糖尿病** 推斷管線尚未完成執行，請等候完成。 然後將 [執行 Python 指令碼] 元件的 [結果資料集] 輸出視覺化，以查看輸入資料中三個病患觀察的預測標籤和機率。
2. 在右上方，按一下 [部署]，然後使用下列設定部署新的即時端點：
    -  **名稱**：designer-predict-diabetes
    -  **描述**：預測糖尿病。
    - **計算類型**：Azure 容器執行個體
3. 等待系統部署 Web 服務 - 這可能需要幾分鐘的時間。 部署狀態會顯示在設計工具介面的左上方。
...
3. 在瀏覽器中開啟 **desinger-predict-diabetes** 服務頁面的 [取用] 頁面之後，就會開啟新的瀏覽器索引標籤，並開啟 Azure Machine Learning 工作室的另一個執行個體。 然後，在新的索引標籤中，檢視 [筆記本] 頁面。
